...
const AdminDashboard = () => {
  ...
  useEffect(() => {
    fetchRelayTeams();
    fetchJerseyNumbers();
    fetchStudents(); // 👈 Fetch students from /students route
  ...
  const fetchJerseyNumbers = async () => {
    try {
      const res = await axios.get("http://localhost:5000/admin/jersey-numbers", {
        withCredentials: true,
      });

      if (res.data.success) {
        setJerseyData(res.data.jerseys);
      }
    } catch (error) {
  ...
  return (
    <div className="bg-light min-vh-100 d-flex flex-column">
      <nav className="navbar navbar-expand-lg navbar-dark bg-dark shadow-sm">
        <div className="container-fluid">
          ...
          <div>
            ...
            <Link to="/export" className="btn btn-primary me-2">
              Export Data
            </Link>
      ...
      <div className="container-fluid flex-grow-1 d-flex flex-column">
        <h2 className="text-center fw-bold text-dark mt-3">
        ...
        <div className="overflow-auto flex-grow-1 px-3">
          {relayTeams.length > 0 ? (
            relayTeams.map((team, index) => (
              <div key={index} className="card shadow-sm mb-4 border-0 w-100">
